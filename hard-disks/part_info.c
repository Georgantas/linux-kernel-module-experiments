...
// Usable hard disk size in bytes = (Number of heads or disks)
// * (Number of tracks per disk) (Number of sectors per track)
// * (Number of bytes per sector, i.e. sector size).
...
// start of partition table, computed by:
// 512 - (4 * 16 + 2 + 6)
// 512: sector size, 4: number of entries
// 16: PARTITION_ENTRY_SIZE, 2: two-byte signature
// 6: more signature?
...
// a partition entry
// if these fields are insufficient to represent the number of
// cylinders on disk, <head, cylinder, sector> fields are set
// to max, and abs_start_sec and sec_in_part are used instead
typedef struct
{
